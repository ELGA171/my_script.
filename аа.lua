-- =============================================
-- Xeno Ultimate Hub v1.0 (ИСПРАВЛЕННЫЙ)
...
local camera = workspace.CurrentCamera  -- <-- ЭТО БЫЛО ДОБАВЛЕНО
...
task.spawn(function()
    while true do
        ...
        -- Aimbot (исправлен, чтобы не телепортировало)
        if aimbotActive then
            local closest = nil
            ...
            if closest then
                ...
                hrp.CFrame = hrp.CFrame:Lerp(newCFrame, 0.3) -- плавное наведение
            end
        end
    end
end)
